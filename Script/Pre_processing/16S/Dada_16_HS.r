...
## Preprocessing of 'Hiddensea_Study'
...
path <- "/home/rstudio/Downloads/Hiddensea/16S"
list.files(path)
...
# Create a path for filtered files
path2 <- "/home/rstudio/Downloads"

# Place filtered files in the filtered/ subdirectory
filtFs <- file.path(path2, "filtered_16S_hiddensea", paste0(sample.names, "_F_filt.fastq.bz2"))
filtRs <- file.path(path2, "filtered_16S_hiddensea", paste0(sample.names, "_R_filt.fastq.bz2"))
...
saveRDS(seqtab, file = paste0(output_dir, "/Seqtab_16S_HS.rds"))
