...
seqtab_Sal <- readRDS("/home/rstudio/Documents/Seqtab/Seqtab_18S_Salinity.rds")
...
all_18S_phy_Sal <- phyloseq(otu_table(seqtab.nochim_Sal, taxa_are_rows=FALSE), refseq(dna_Sal))

# Change ASV names from the sequence to ASV0001 ... ASV31182
taxa_names(all_18S_phy_Sal) <- paste0("ASV", seq(ntaxa(all_18S_phy_Sal)))

# Export ASV and sequence table for classification with CREST4
writeXStringSet(refseq(all_18S_phy_Sal), "all_seqs_18S_Sal.fasta", format = "fasta")
asv_table_df_t_Sal <- t(as.data.frame(otu_table(all_18S_phy_Sal)))
write.csv(asv_table_df_t_Sal, "all_asv_18S_counts_Sal.csv")

# Save the workspace
save.image("all_18S_Sal.RData")
...
assign <- read.table(text = gsub("\t|; ", ";", readLines("~/all_seqs_18S_Sal.fasta.crest4/assignments.txt")), 
...
pro_otu <- read.csv("~/all_asv_18S_counts_Sal.csv", header = TRUE, row.names = 1, sep= ",")
...
phys_Sal <- merge_phyloseq(pro_OTU, tax_phy, all_18S_phy_Sal, sample_data(meta))
...
# Save the workspace
save.image("~/Documents/workspace/18S_AM_Sal")

# Define the output directory
output_dir <- "/home/rstudio/Documents/phyloseq"

# Save the phyloseq object in RDS format
saveRDS(phys_Sal, file = paste0(output_dir, "/Phyloseq_Sal_18S.rds"))