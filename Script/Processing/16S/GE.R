...
## Processing of 'Germination_Experiment'
...
# Merge multiple runs 16S sequence table
seqtab_Germ <- readRDS("/home/rstudio/Documents/Seqtab/Seqtab_16S_Germ.rds")
...
phy_16S_Germ <- phyloseq(otu_table(seqtab.nochim_Germ, taxa_are_rows=FALSE), refseq(dna_Germ))

# Change ASV names from the sequence to ASV0001 ... ASV31182
taxa_names(phy_16S_Germ) <- paste0("ASV", seq(ntaxa(phy_16S_Germ)))

# Export ASV and sequence table for classification with CREST4
writeXStringSet(refseq(phy_16S_Germ), "Seqs_16S_Germ.fasta", format = "fasta")
asv_table_df_t_Germ <- t(as.data.frame(otu_table(phy_16S_Germ)))
write.csv(asv_table_df_t_Germ, "16S_counts_Germ.csv")

# Save the workspace
save.image("16S_Germ.RData")
...
assign <- read.table(text = gsub("\t|; ", ";", readLines("~/Documents/fasta_crest4/Seqs_16S_Germ.fasta.crest4/assignments.txt")), 
...
pro_otu <- read.csv("~/16S_counts_Germ.csv", header = TRUE, row.names = 1, sep= ",")
...
sample_names <- sapply(strsplit(sample_names, "515F-806R-P01-"), `[`, 2)
...
phys_Germ <- merge_phyloseq(pro_OTU, tax_phy, phy_16S_Germ, sample_data(meta))
sample_names(phys_Germ) <- sample_data(phys_Germ)$Sample_ID
print(phys_Germ)
...
# Save the workspace
save.image("~/Documents/workspace/16S_AM_Germ.RData")

# Save the phyloseq object in RDS format
saveRDS(phys_Germ, file = paste0(output_dir, "/Phyloseq_Germ_16S.rds"))
